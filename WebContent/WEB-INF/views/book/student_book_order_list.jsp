	...
	<div class="container">
        ...
        <tbody>
          <c:forEach items="${bookOrderList}" var="order" varStatus="row">
							<tr>
								<td>${row.count}</td>
								<td class="hidden-480">${order.studentName}</td>
								<td class="hidden-480">${order.invoiceNo}</td>
								<td class="hidden-480">${order.totalPrice}</td>
								<td><a href="<c:url value='/book/order_detail.htm/${order.id}' />">Details</a></td>
							</tr>
						</c:forEach> 
        </tbody>
        <tfoot>
			...
			success : function(data){
				...
				$.each(data, function( key, value ) {
					/* var row = table.insertRow(rowCnt);
					  ...
					  */
					  str+="<tr><td>"+value.name+"</td><td>"+value.qty+"</td><td>"+value.price+"</td></tr>";
