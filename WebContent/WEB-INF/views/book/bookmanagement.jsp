...
<body>
	<div id="right-panel" class="right-panel" style="width: 100%;">
        ...
        <tbody>
          <c:forEach items="${bookList}" var="item" varStatus="row">
							<tr>
								<td>${row.count}</td>
								<td class="hidden-480">${item.name}</td>
								<td class="hidden-480">${item.price}</td>
								<td class="hidden-480">${item.author}</td>
								<td class="hidden-480">${item.isbn}</td>
								<td class="hidden-480">${item.edition}</td>
...
<div class="modal-dialog modal-notify modal-warning" role="document">
	...
	$.ajax({
		...
		type : "GET",
		url : contextUrl+"/book/ajax/delete-book/"+id,
		dataType : 'json',
		success : function(data){
			if(data+""=== "true"){
				document.location.href='bookManagement.htm';
			}else{
				alert("Cannot Delete");
			}
