         ...
         <div class="col-lg-12">
			      <div class="table-responsive">
			        <table class="table table-hover">
			          ...
			          <tbody>
			           <!--   for using loop -->
			           <c:forEach items="${documentList}" var="item" varStatus="row">
							<tr>
								<td>${row.count}</td>
								<td class="hidden-480">${item.name}</td>
								<td class="hidden-480">${item.subjectName}</td>
								<td class="hidden-480">${item.teacherName}</td>
								<td>
								<a class="btn btn-outline-light btn-sm" href="<c:url value='/teacher/detailLessonPlan.htm/${item.id}'/>">View</a></td>
								</tr>
								</c:forEach>
			            <!--  for using loop -->
