(function(g) {
	"function" === typeof define && define.amd ? define([ "jquery", "datatables.net", "datatables.net-buttons" ], function(k) {
		...
		l.fn.dataTable.Buttons || require("datatables.net-buttons")(k, l);return g(l, k, k.document)
	...
	function x(a) {
		r === q && (r = -1 === v.serializeToString(g.parseXML(n["xl/worksheets/sheet1.xml"])).indexOf("xmlns:r"));g.each(a, function(b, d) {
			if (g.isPlainObject(d)) x(d);else {
				...
				c = v.serializeToString(d);r && (-1 === c.indexOf("<?xml") && (c = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + c), c = c.replace(/_dt_b_namespace_token_/g, ":"));
	...
	var j = g.fn.dataTable,
		h = {
			...
			nextId : 1,
			$ : function(a) {
				"string" == typeof a && (a = l.getElementById(a));a.addClass || (a.hide = function() {
	...
	j.Buttons.swfPath = "//cdn.datatables.net/buttons/" +
		j.Buttons.version + "/swf/flashExport.swf";j.Api.register("buttons.resize()", function() {
		...
		action : function(a, b, d, c) {
			if (c._fromFlash) {
				...
				var a = c._flash,
					...
					f = z(c),
					e = e.str;
				d.title && (e = d.title + f + f + e);d.messageTop && (e = d.messageTop +
	...
	j.ext.buttons.csvFlash = g.extend({}, u, {
		...
		action : function(a, b, d, c) {
			...
			a.setAction("csv");a.setFileName(d.filename);t(a, b)
		},
		escapeChar : '"'
	});
	j.ext.buttons.excelFlash = g.extend({}, u, {
		...
		action : function(a, b, d, c) {
					...
					for (var b = 0, d = a.length; b < d; b++) {
						...
						for (var m = 0, p = B.length; m < p; m++) {
							...
							if (a[b].match && !a[b].match(/^0\d+/) &&
								a[b].match(n.match)) {
								i = a[b].replace(/[^\d\.\-]/g, "");n.fmt && (i = n.fmt(i));
								i = o(f, "c", {
									attr : {
										r : j,
										s : n.style
			...
			g("sheets sheet", d.xl["workbook.xml"]).attr("name", D(c));c.customizeData && c.customizeData(i);
			var m = function(a, b) {
					...
					}));c.attr("count", c.attr("count") + 1);g("row:eq(" + (a - 1) + ") c", f).attr("s", "51")
				},
				p = b.buttons.exportInfo(c);
			p.title && (j([ p.title ], e), m(e, i.header.length - 1));p.messageTop && (j([ p.messageTop ], e), m(e, i.header.length - 1));c.header && (j(i.header, e), g("row:last c", f).attr("s", "2"));
			for (var s = 0, r = i.body.length; s <
				r; s++) j(i.body[s], e);
			c.footer && i.footer && (j(i.footer, e), g("row:last c", f).attr("s", "2"));p.messageBottom && (j([ p.messageBottom ], e), m(e, i.header.length - 1));
			j = o(f, "cols");g("worksheet", f).prepend(j);
			m = 0;
			...
			c.customize && c.customize(d, c, b);x(d);a.setAction("excel");a.setFileName(p.filename);a.setSheetData(d);t(a, "");this.processing(!1)
		},
		extension : ".xlsx",
		createEmptyCells : !1
	});
	j.ext.buttons.pdfFlash = g.extend({}, u, {
		...
		action : function(a, b, d, c) {
			...
			a.setAction("pdf");a.setFileName(e.filename);t(a, JSON.stringify({
				title : e.title || "",
